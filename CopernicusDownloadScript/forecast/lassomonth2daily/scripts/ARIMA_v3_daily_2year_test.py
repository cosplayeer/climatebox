...
def getdf():
    ...
    # 02-30->0x-30
    # 31*4->31x4
    # flists = ["jianggongling2018-2021obs6hourlyUTC0month1.csv"]
    flists = ["jianggongling2018-2021obs6hourlyUTC0month1.csv"]
...
def getdf10m():
    ...
    flists = ["ecmwf_jianggongling_6.csv"]
...
def main():
    ...
    # 数据预处理
    ts_log = np.log(ts)
    # rol_mean = ts_log.rolling(window=31*4).mean()
 ...
 # 6.完善ＡＲＩＭＡ模型
    ...
    # 差分数据处理
    diffed_ts = diff_ts(ts_log, d=[31*4, 1])
    model = arima_model(diffed_ts)
    # # model.certain_model(0, 1)
    model.get_proper_model()
    predict_ts = model.properModel.predict()
    diff_recover_ts = predict_diff_recover(predict_ts, d=[31*4, 1])
    log_recover = np.exp(diff_recover_ts)
    log_recover_train = log_recover[:'2020-01-31 18:00:00']  # dyp
    # # print(ts_log)
    ...
    plt.savefig('pic/Figure_wind_2021_test_jianggongling_month1_3year.png')
    ...
    ts_train = ts_log[:'2020-01-31 18:00:00']
    ts_test = ts_log['2021-01-01 00:00:00':]

    diffed_ts = diff_ts(ts_train, [31*4, 1])
    forecast_list = []
    ...
    for i, dta in enumerate(ts_test):
        ...
        add_today_data(model, ts_train, dta, [31*4, 1], type='month')
    ...
    plt.savefig('pic/Figure_wind_2021_valid_jianggongling_month1_3year.png')
    ...
    outpath = './text/'
    filename = 'arima_out_wind202102_jianggongling_month1_3year.csv'
