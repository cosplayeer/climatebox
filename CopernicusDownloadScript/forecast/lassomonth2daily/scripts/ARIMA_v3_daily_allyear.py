...
def getdf():
    ...
    # flists = ["diqingyuan2018-2021obs6hourlyUTC0allyear.csv"]
    flists = ["jianggongling2018-2021obs6hourlyUTC0.csv"]
...
def main():
    ...
    # 数据预处理
    ts_log = np.log(ts)
    rol_mean = ts_log.rolling(window=724).mean()
...
# dyp common
 # 6.完善ＡＲＩＭＡ模型
    ...
    # # 差分数据处理
    # diffed_ts = diff_ts(ts_log, d=[724, 1])
    # model = arima_model(diffed_ts)
    # # model.certain_model(0, 2)
    # model.get_proper_model()

    # predict_ts = model.properModel.predict()
    # diff_recover_ts = predict_diff_recover(predict_ts, d=[724, 1])
    # log_recover = np.exp(diff_recover_ts)
    # log_recover_train = log_recover[:'2020-06-30 18:00:00']
    # # # print(ts_log)
    ...
    # plt.savefig('pic/Figure_wind_2021_train_diqingyuan_harfyear.png')
    ...
    # ts_train = ts_log[:'2020-06-30 18:00:00']
    # ts_test = ts_log['2021-01-01 00:00:00':]

    # diffed_ts = diff_ts(ts_train, [724, 1])
    # forecast_list = []
    ...
    #     add_today_data(model, ts_train, dta, [724, 1], type='month')

    # predict_ts = pd.Series(
    #     data=forecast_list, index=ts['2021-01-01 00:00:00':].index)
    # log_recover = np.exp(predict_ts)
    # original_ts = ts['2021-01-01 00:00:00':]
    ...
    #     sum((log_recover-ts)**2)/ts.size), np.sqrt(sum((log_recover-ts2)**2)/ts.size)))
    # plt.xticks(rotation=20)
    # plt.savefig('pic/Figure_wind_2021_valid__diqingyuan_harfyear.png')
    ...
    # outpath = './text/'
    # filename = 'arima_out_wind2012102_diqingyuan_harfyear.csv'
