...
def lasso_model(f1, f2, ilat, jlon):
    # 1. 读入实测
    ...
    for i in range(21):
        spd10mObsArray[i] = spd10mObs[i*6]  # 从第六行开始，每次跳过６行，即为１月份结果
...
def circle():
    ...
    # print("1...type matrix is: {}".format(type(test_matrix)))
    for i in range(nlat):
        for j in range(nlon):
            _model = lasso_model(f1=f1, f2=f2, ilat=i, jlon=j)
            print("ilat:%s" % i)
            print("jlon:%s" % j)
...
def outputnc(varmatrix):
    ...
    os.system("rm -rf data_0.25_2022/wind202201.nc")
    outf = Nio.open_file("data_0.25_2022/wind202201.nc", "c")
