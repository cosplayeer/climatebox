...
class Radiation:
    ...
    def readinERA5AndCFSv2(self, fnameERA5, fnameCFS):
        ...
        df = pd.concat([df_era5, df_cams], axis=1)
        df['GHI_era5_cams'] = (df['GHI_ERA5_ssrc'] + df['GHI_CFSv2']) / 2
        df2 = df.drop(labels='GHI_ERA5_ssrc', axis=1)
    ...
    def processSumMonth(self, fname):
        s = dict()
        if self.dtype == 'obs':
        ...
        # s to csv
        s['13'] = '{:.2f}'.format(self.processSumYear(fname))
        s2 = pd.DataFrame.from_dict(s, orient='index')
        s2.to_csv('text/Monthly'+fname)
        # return s

    def processSumYear(self, fname):
        if self.dtype == 'obs':
            df = self.readinObs(fname)
